---
layout: post
title:  "Variational AutoeEncoder is not an AutoEncoder_1"
...
즉 x가 evidence로 주어질 때 적어도 x는 잘 만들어내는 z를 sampling하는 분포를 찾기 위해 p(z|x)를 찾는다.     
...
이제, p(x)와 p(z|x),  $q_{\phi}(z \mid x)$의 관계를 식으로 알아보자.
...
이상적인 sampling함수 $p(z|x)$와 이에 대한 근사함수인 $q_{\phi}(z|x)$이 가까울수록, 즉 분포를 잘 근사했을수록 KL이 작아진다.  
...
$= E_{q_{\phi(z|x)}}[log(p(x|z))]-KL(q_{\phi}(z|x)\parallel p(z))$  
...
즉 $E_{q_{\phi(z|x)}}[log(p(x|g_{\theta}(z)))]$가 커지기를 바란다.

그런데 $ELBO(\phi)$ 안의 $E_{q_{\phi(z|x)}}[log(p(x|z))]$가  이와 구조가 동일하다.
